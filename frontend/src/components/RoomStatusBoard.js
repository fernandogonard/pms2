...
// Configuración para limpiezas diferenciadas
const CLEANING_CONFIG = {
  repaso:             { label: '🧹 Repaso rápido',     color: '#60a5fa', bg: 'rgba(96,165,250,0.15)' },
  limpieza_profunda:  { label: '🧼 Limpieza profunda', color: '#a78bfa', bg: 'rgba(167,139,250,0.15)' },
  limpieza_checkout:  { label: '🏃 Checkout limpieza', color: '#f97316', bg: 'rgba(249,115,22,0.15)' },
};
...
const RoomCard = ({ room, reservation, onPay, onCharge }) => {
  ...
  // Info de checkout si está disponible
  const checkoutInfo = room.checkoutToday ? room.checkoutInfo : null;
  const cleaningAssignment = room.housekeepingAssignment;
  const cleaningType = room.pendingHousekeeping ? CLEANING_CONFIG[room.pendingHousekeeping] : null;

  const total       = reservation?.pricing?.total      || checkoutInfo?.totalAmount || 0;
  const paid        = reservation?.payment?.amountPaid || checkoutInfo?.amountPaid || 0;
  ...
  const guestName = checkoutInfo?.guestName ||
    (reservation?.client
  ...
  const nights = checkoutInfo?.nightsStayed ||
    (reservation
  ...
  return (
    <div style={{
      ...
      <div style={{ padding:'14px 16px' }}>
        {(reservation && guestName) || checkoutInfo ? (
          <>
            {/* Huésped y fechas */}
            <div style={{ marginBottom:10 }}>
              <div style={{ color:'#fff', fontWeight:700, fontSize:14 }}>👤 {guestName}</div>
              <div style={{ color:'#9ca3af', fontSize:12, marginTop:2 }}>
                {reservation?.checkIn?.slice(0,10) || checkoutInfo?.checkoutDate} → {reservation?.checkOut?.slice(0,10) || checkoutInfo?.checkoutDate}
                {nights ? <span style={{ marginLeft:6, color:'#60a5fa' }}>({nights} noche{nights !== 1 ? 's' : ''})</span> : null}
              </div>
            </div>

            {/* AVISO DE PAGO PENDIENTE */}
            {room.checkoutToday && checkoutInfo && !checkoutInfo.isPaid && (
              <div style={{
            ...
            {/* ESTADO DE LIMPIEZA PARA CHECKOUTS */}
            {room.checkoutToday && cleaningAssignment && (
              <div style={{
                background: cleaningType?.bg || 'rgba(167,139,250,0.15)',
                border: `1px solid ${cleaningType?.color || '#a78bfa'}40`,
                borderRadius: 8,
                padding: '8px 12px',
                marginBottom: 10,
                fontSize: 12,
                color: cleaningType?.color || '#a78bfa'
              }}>
                <div style={{ fontWeight: 700, marginBottom: 4 }}>
                  {cleaningType?.label || '🧹 Pendiente'}
                </div>
                <div style={{ fontSize: 11, opacity: 0.8 }}>
                  {cleaningAssignment.status === 'no_asignada' && '❌ Sin asignar'}
                  {cleaningAssignment.status === 'asignada' && `📌 Asignado a ${cleaningAssignment.assignedTo}`}
                  {cleaningAssignment.status === 'en_progreso' && `🧹 En progreso - ${cleaningAssignment.estimatedDurationMinutes || '?'} min`}
                  {cleaningAssignment.status === 'completada' && '✨ Completada'}
                </div>
              </div>
            )}
...
const RoomStatusBoard = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Obtener también checkouts de hoy para popul habitaciones
      let checkoutRooms = [];
      try {
        const checkoutRes = await apiFetch('/api/cleaning/checkouts/today');
        const checkoutData = await checkoutRes.json();
        checkoutRooms = Array.isArray(checkoutData?.data) ? checkoutData.data : [];
      } catch {
        // Si falla la llamada de checkouts, continuar sin ellos
      }
      
      // Combinar datos: si hay checkoutToday, usar datos del checkout
